...
bool depth_driven::consistent(state_merger *merger, apta_node* left, apta_node* right){
    ...
    if(left->pos_paths() < STATE_COUNT || right->pos_paths() < STATE_COUNT) return true;
...
void depth_driven::update_score(state_merger *merger, apta_node* left, apta_node* right){
    ...
    int error_left  = left->pos_paths()  - max_left;
    int error_right = right->pos_paths() - max_right;
    int error_total = left->pos_paths() + right->pos_paths() - max_total;
