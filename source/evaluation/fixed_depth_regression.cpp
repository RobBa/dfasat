...
double fixed_depth_mse_error::compute_score(state_merger *merger, apta_node* left, apta_node* right){
    ...
    for(int i = 0; i < merger->aut->max_depth; ++i){
        for(int a = 0; a < alphabet_size; ++a){
            ...
            for(state_set::iterator it = l_dist[i].begin(); it != l_dist[i].end(); ++it){
                ...
                count_left += l->ald.count_symbol(a);
                total_left += l->ald.pos_paths() + l->ald.neg_paths();
            }
            for(state_set::iterator it = r_dist[i].begin(); it != r_dist[i].end(); ++it){
                apta_node* node = *it;
                fixed_depth_mse_data* r = (fixed_depth_mse_data*)node->data;
                
                count_right += r->ald.count_symbol(a);
                total_right += r->ald.pos_paths() + r->ald.neg_paths();
            }
...
//    fprintf(output,"subgraph cluster_1 {\nstyle=filled;color=green4;");
    ...
    for(state_set::iterator it = states.begin(); it != states.end(); ++it){
        apta_node* node = *it;
        fixed_depth_mse_data* l = (fixed_depth_mse_data*) node->data;
        if (sink_type(node) == -1 && node->depth != aut->max_depth && l->ald.pos_paths() > 2.0 * l->ald.neg_paths()){// || l->pos_final() != 0){
            output << node->number << ";";
        }
    }
...
//    fprintf(output,"subgraph cluster_2 {\nstyle=filled;color=purple4;");
    ...
    for(state_set::iterator it = states.begin(); it != states.end(); ++it){
        apta_node* node = *it;
        fixed_depth_mse_data* l = (fixed_depth_mse_data*) node->data;
        if (sink_type(node) == -1 &&node->depth != aut->max_depth &&  l->ald.pos_paths() <= 2.0 * l->ald.neg_paths()
        && l->ald.neg_paths() <= 2.0 * l->ald.pos_paths()){// || l->pos_final() != 0){
            output << node->number << ";";
        }
    }
...
//    fprintf(output,"subgraph cluster_3 {\nstyle=filled;color=red4;");
    ...
    for(state_set::iterator it = states.begin(); it != states.end(); ++it){
        apta_node* node = *it;
        fixed_depth_mse_data* l = (fixed_depth_mse_data*) node->data;
        if (sink_type(node) == -1 && node->depth != aut->max_depth && l->ald.neg_paths() > 2.0 * l->ald.pos_paths()){// || l->neg_final() != 0){
            output << node->number << ";";
        }
    }
...
//    fprintf(output,"}\n");
    ...
    for(state_set::iterator it = merger->red_states.begin(); it != merger->red_states.end(); ++it){
        ...
        if(l->ald.pos_final() != 0)
            ...
            output << ")\n" << (float)((int)(100*l->mean))/100.0 << "\n" << (int)l->occs.size() << "\n" << l->ald.pos_paths() << ":" << l->ald.neg_paths() << "\"];\n";
            //else
