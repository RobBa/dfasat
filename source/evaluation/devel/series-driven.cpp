...
bool series_driven::consistent(state_merger *merger, apta_node* left, apta_node* right){
  ...
  int total_left = left->pos_paths() + left->neg_paths();
  int total_right = right->pos_paths() + right->neg_paths();
...
/*  //if(total_left >= STATE_COUNT && total_right >= STATE_COUNT){
  ...
  return distance;*/
    ...
    //cerr << "mse:" << mse << endl;
    //if(left->pos_paths() +  left->neg_paths() < STATE_COUNT || right->pos_paths() +  right->neg_paths() < STATE_COUNT)
    //    return -1;
