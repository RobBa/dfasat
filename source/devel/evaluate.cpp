...
bool is_low_count_sink(apta_node* node){
    node = node->find();
    return node->pos_paths() + node->pos_final() < STATE_COUNT;
}
...
bool sink_consistent(apta_node* node, int type){
    ...
    if(type == 0) return node->neg_paths() == 0 && node->neg_final() == 0;
    if(type == 1) return node->pos_paths() == 0 && node->pos_final() == 0;
    return true;
}
...
double depth_driven::compute_score(state_merger *merger, apta_node* left, apta_node* right){
  return depth;
};
...
bool overlap_driven::consistent(state_merger *merger, apta_node* left, apta_node* right){
  ...
  if(left->pos_paths() >= STATE_COUNT && right->pos_paths() >= STATE_COUNT){
    int total_left = left->pos_paths() + left->pos_final();
    int total_right = right->pos_paths() + left->pos_final();
    ...
    for(int i = 0; i < alphabet_size; ++i){
      ...
      if(left->pos_final() >= SYMBOL_COUNT || right->pos_final() >= SYMBOL_COUNT){
        double gamma = 0.0;
        if( ((double)left->pos_final() / (double)total_left) >
            ((double)right->pos_final() / (double)total_right) )
          gamma = ((double)left->pos_final() / (double)total_left)
                - ((double)right->pos_final() / (double)total_right);
        else
          gamma = ((double)right->pos_final() / (double)total_right)
                - ((double)left->pos_final() / (double)total_left);
        if(gamma > bound){ inconsistency_found = true; return false; }
      }
  }
  return true;
  
if(left->pos_paths() >= STATE_COUNT){
  ...
  if(right->pos_paths() >= STATE_COUNT){
...
double series_driven::compute_score(state_merger *merger, apta_node* left, apta_node* right){
...
double metric_driven::compute_score(state_merger *merger, apta_node* left, apta_node* right){
  return overlap;
};
...
bool alergia::consistent(state_merger *merger, apta_node* left, apta_node* right){
  ...
  if(left->pos_paths() >= STATE_COUNT && right->pos_paths() >= STATE_COUNT){
    double bound = sqrt(1.0 / (double)left->pos_paths()) 
                 + sqrt(1.0 / (double)right->pos_paths());
    bound = bound * sqrt(0.5 * log(2.0 / CHECK_PARAMETER));
    
    for(int i = 0; i < alphabet_size; ++i){
      if(left->pos(i) >= SYMBOL_COUNT || right->pos(i) >= SYMBOL_COUNT){
        double gamma = 0.0;
        if( ((double)left->pos(i) / (double)left->pos_paths()) >
            ((double)right->pos(i) / (double)right->pos_paths()) )
          gamma = ((double)left->pos(i) / (double)left->pos_paths())
                - ((double)right->pos(i) / (double)right->pos_paths());
        else
          gamma = ((double)right->pos(i) / (double)right->pos_paths())
                - ((double)left->pos(i) / (double)left->pos_paths());
        if(gamma > bound){ inconsistency_found = true; return false; }
      }
    }
  }
...
void likelihoodratio::update_score(state_merger *merger, apta_node* left, apta_node* right){
  if(right->pos_paths() < STATE_COUNT || left->pos_paths() < STATE_COUNT) return;
  ...
  left_divider = (double)left->pos_paths() + corr;
  right_divider = (double)right->pos_paths() + corr;
